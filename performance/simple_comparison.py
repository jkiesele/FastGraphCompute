...
def time_fgc(data, k):
    ...
    try:
        ...
        indices, distances = torch.ops.binned_select_knn.binned_select_knn(k, coordinates, row_splits)
        if device == 'cuda':
            torch.cuda.synchronize()
        end_time = time.time()
...
def main():
    ...
    for size in TEST_SIZES:
        ...
        fgc_str = f"{fgc_time:.2f}" if isinstance(fgc_time, (int, float)) else "ERROR"
