...
def benchmark_oc_helper_performance(device='cpu', num_nodes=1000000, num_row_splits=5, num_unique_per_split=1000, num_runs=5):
    ...
    asso_indices = torch.randint(
        0, num_unique_per_split, (num_nodes,), dtype=torch.int64, device=device) - 1
    ...
    row_splits = torch.arange(
        0, num_nodes + 1, split_size, dtype=torch.int64, device=device)
...
if __name__ == '__main__':
    ...
    if True:
        ...
        cpu_benchmark = benchmark_oc_helper_performance(
            device='cpu', num_nodes=1000000, num_row_splits=5, num_unique_per_split=100, num_runs=5)
        print(cpu_benchmark)
        cpu_benchmark = benchmark_oc_helper_performance(
            device='cpu', num_nodes=200, num_row_splits=2, num_unique_per_split=300, num_runs=5)
        print(cpu_benchmark)

    # Run the benchmark on CUDA (if available)
    if torch.cuda.is_available():
        print("Benchmarking on CUDA")
        cuda_benchmark = benchmark_oc_helper_performance(
            device='cuda', num_nodes=1000000, num_row_splits=5, num_unique_per_split=1000, num_runs=5)
        print(cuda_benchmark)
        cuda_benchmark = benchmark_oc_helper_performance(
            device='cuda', num_nodes=1000000, num_row_splits=5, num_unique_per_split=100, num_runs=5)
        print(cuda_benchmark)
        cuda_benchmark = benchmark_oc_helper_performance(
            device='cuda', num_nodes=200000, num_row_splits=1, num_unique_per_split=1000, num_runs=5)
        print(cuda_benchmark)
        cuda_benchmark = benchmark_oc_helper_performance(
            device='cuda', num_nodes=200000, num_row_splits=1, num_unique_per_split=100, num_runs=5)
        print(cuda_benchmark)
        cuda_benchmark = benchmark_oc_helper_performance(
            device='cuda', num_nodes=1000000, num_row_splits=20, num_unique_per_split=100, num_runs=5)
        print(cuda_benchmark)
