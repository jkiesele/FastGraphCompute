...
class TestIndexReplacer(unittest.TestCase):
    ...
    def run_test(self, device):
        ...
        to_be_replaced = torch.tensor(tbr, dtype=torch.int32, device=device)

        #define replacements
        rpl = np.arange(100)
        np.random.shuffle(rpl)
        replacements = torch.tensor(rpl, dtype=torch.int32, device=device)

        # Call the function
        replaced = index_replacer(to_be_replaced, replacements)

        #check if device is correct
        self.assertTrue(replaced.device.type == device, "Test index replacer device failed: device wrong")

        #check if dtype is same as to_be_replaced
        self.assertTrue(replaced.dtype == to_be_replaced.dtype, "Test index replacer dtype failed: dtype wrong")
        
        #check if -1 are at the same place in the replaced array
        noise_ok = to_be_replaced[to_be_replaced<0] == replaced[to_be_replaced<0]

        self.assertTrue(torch.all(noise_ok), "Test index replacer noise failed")
        ...
        self.assertTrue(torch.all(ref_array == replaced), "Test index replacer failed")
