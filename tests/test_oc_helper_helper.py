...
class TestObjectCondensationHelperHelper(unittest.TestCase):
    ...
    def test_unique_with_counts(self):
        x = torch.tensor([1, 2, 3, 4, 1, 2, 3, 4], dtype=torch.int32)
        ...
        #print both
        self.assertTrue(torch.equal(unique, expected_unique), f'{unique=}, {expected_unique=}')
        self.assertTrue(torch.equal(counts, expected_counts), f"{counts=}, {expected_counts=}")
    ...
    def test_negative_values(self):
        x = torch.tensor([-3, -1, -2, -3, -1, -2, -3, -1], dtype=torch.int32)
        unique, counts = torch.ops.oc_helper_helper.unique_with_counts(x)
        expected_unique, expected_counts = torch.unique(x, return_counts=True)
        self.assertTrue(torch.equal(unique, expected_unique))
        self.assertTrue(torch.equal(counts, expected_counts))
    ...
    def test_single_element_tensor(self):
        x = torch.tensor([42], dtype=torch.int32)
        unique, counts = torch.ops.oc_helper_helper.unique_with_counts(x)
        expected_unique, expected_counts = torch.unique(x, return_counts=True)
        self.assertTrue(torch.equal(unique, expected_unique))
        self.assertTrue(torch.equal(counts, expected_counts))
    ...
    def test_cpp_vs_python(self):
        ...
        row_splits = torch.tensor([0, 9, len(asso_idx)], dtype=torch.int32)

        r_mps, r_gm, r_ops = self.ref_max_same_valued_entries_per_row_split(asso_idx, row_splits, True)

        o_mps, o_gm, o_ops = torch.ops.oc_helper_helper.max_same_valued_entries_per_row_split(asso_idx, row_splits, True)

        self.assertTrue(torch.equal(r_mps, o_mps), f'{o_mps=}, {r_mps=}, {o_gm=}, {r_gm=}, {o_ops=}, {r_ops=}')
        self.assertTrue(torch.equal(r_gm, o_gm),  f'{o_mps=}, {r_mps=}, {o_gm=}, {r_gm=}, {o_ops=}, {r_ops=}')
        self.assertTrue(torch.equal(r_ops, o_ops),  f'{o_mps=}, {r_mps=}, {o_gm=}, {r_gm=}, {o_ops=}, {r_ops=}')
