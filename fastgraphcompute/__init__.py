...
build_dirs = glob.glob(os.path.join(os.path.dirname(extensions_dir), 'build', '*'))
for build_dir in build_dirs:
    for lib_pattern in ['*.so', '*.dylib', '*.dll']:
        for lib_path in glob.glob(os.path.join(build_dir, lib_pattern)):
            try:
                torch.ops.load_library(lib_path)
                print(f'Loaded extension from build dir: {os.path.basename(lib_path)}')
            except Exception as e:
                print(f'Failed to load {os.path.basename(lib_path)} from build dir: {e}')
...
from .extensions.binned_knn_forward_cpu import binned_knn_forward_cpu
from .extensions.binned_knn_forward_cuda import binned_knn_forward_cuda
