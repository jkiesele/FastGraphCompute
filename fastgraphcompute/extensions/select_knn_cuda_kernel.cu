    ...
    int64_t mask_mode)
{
    ...
    auto stream = at::cuda::getCurrentCUDAStream(coords.device().index());
    ...
    auto err = cudaMemcpyAsync(cpu_rowsplits.data(), row_splits.data_ptr<int64_t>(), n_rs * sizeof(int64_t), cudaMemcpyDeviceToHost, stream.stream());
